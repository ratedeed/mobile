...
const TIME_OPTIONS = [
  '00:00', '01:00', '02:00', '03:00', '04:00', '05:00', '06:00', '07:00', '08:00', '08:30', '09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '12:00', '12:30', '13:00', '13:30', '14:00', '14:30', '15:00', '15:30', '16:00', '16:30', '17:00', '17:30', '18:00', '18:30', '19:00', '19:30', '20:00', '20:30', '21:00', '22:00', '23:00'
];
...
const SectionHeading = ({ title, subtitle }: { title: string; subtitle?: string }) => (
  <View style={{ marginBottom: 20 }}>
    <Text style={{ fontSize: 22, fontWeight: '700', color: COLORS.dark, letterSpacing: -0.3 }}>
      {title}
    </Text>
    {subtitle && (
...
export default function ContractorEditProfileModal({ visible, onClose, onProfileUpdated }: Props) {
  ...
  const renderTimePicker = () => {
    ...
    return (
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', zIndex: 9999, elevation: 9999 }}>
        <View style={{ width: '80%', backgroundColor: '#fff', borderRadius: 20, padding: 20, maxHeight: '70%' }}>
          <Text style={{ fontSize: 18, fontWeight: '700', color: COLORS.dark, marginBottom: 16 }}>Select {type === 'open' ? 'Opening' : 'Closing'} Time for {day}</Text>
          <ScrollView>
            {TIME_OPTIONS.map(time => (
              ...
              >
                <Text style={{ fontSize: 16, color: currentValue === time ? COLORS.primary : COLORS.textPrimary, fontWeight: currentValue === time ? '600' : '400' }}>{time}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <TouchableOpacity onPress={() => setShowTimePicker(null)} style={{ marginTop: 16, padding: 12, alignItems: 'center' }}>
  ...
  const renderAccordionHeader = (tabId: TabId, title: string, icon: string) => (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 18, fontWeight: '600', color: activeTab === tabId ? COLORS.primary : COLORS.dark, marginLeft: 16 }}>
          {title}
        </Text>
      </View>
            ...
            <View style={{ flex: 1, backgroundColor: COLORS.background, position: 'relative' }}>
              {loading ? (
                ...
                >
                  ...
                  {activeTab === 'basic' && (
                    <View style={{ gap: 24, paddingBottom: 32, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight }}>
                      <SectionHeading
                    ...
                    <View style={{ gap: 24, paddingBottom: 32, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight }}>
                      <View>
                        ...
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                          {zipCodes.map(z => (
                            <View key={z} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surfaceWarm, paddingHorizontal: 12, paddingVertical: 8, borderRadius: RADII.full, borderWidth: 1, borderColor: COLORS.borderLight }}>
                              <FontAwesome5 name="map-marker-alt" size={10} color={COLORS.textSecondary} style={{ marginRight: 6 }} />
                      ...
                      <View>
                        <SectionHeading title="Your services" subtitle="Add the services you offer so homeowners can find you." />
                        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
                          <TextInput
                            value={serviceInput}
                        ...
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                          {servicesList.map(s => (
                            <View key={s} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surfaceWarm, paddingHorizontal: 12, paddingVertical: 8, borderRadius: RADII.full, borderWidth: 1, borderColor: COLORS.borderLight }}>
                      ...
                      <View>
                        <SectionHeading title="Business Hours" subtitle="Let homeowners know when you're available." />
                        <View style={{ gap: 12 }}>
                          {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => (
                            <View key={day} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight }}>
                              <View style={{ width: 100, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                <TouchableOpacity 
                                  onPress={() => setHours({ ...hours, [day]: { ...hours[day], isOpen: !hours[day]?.isOpen } })}
                                  style={{ width: 20, height: 20, borderRadius: 4, borderWidth: 1.5, borderColor: hours[day]?.isOpen ? COLORS.primary : COLORS.border, backgroundColor: hours[day]?.isOpen ? COLORS.primary : 'transparent', alignItems: 'center', justifyContent: 'center' }}
                                >
                                  {hours[day]?.isOpen && <FontAwesome5 name="check" size={10} color="#fff" />}
                                </TouchableOpacity>
                                <Text style={{ fontSize: 14, color: COLORS.textPrimary, fontWeight: '500' }}>{day}</Text>
                              </View>
                              ...
                              {hours[day]?.isOpen ? (
                                <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, gap: 8 }}>
                                  <TouchableOpacity 
                                    ...
                                    style={{ flex: 1, borderWidth: 1, borderColor: COLORS.border, borderRadius: RADII.sm, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: COLORS.background, alignItems: 'center' }}
                                  >
                                    <Text style={{ fontSize: 13, color: COLORS.textPrimary }}>{hours[day]?.open || '09:00'}</Text>
                                  </TouchableOpacity>
                                  ...
                                  <TouchableOpacity 
                                    ...
                                    style={{ flex: 1, borderWidth: 1, borderColor: COLORS.border, borderRadius: RADII.sm, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: COLORS.background, alignItems: 'center' }}
                                  >
                                    <Text style={{ fontSize: 13, color: COLORS.textPrimary }}>{hours[day]?.close || '17:00'}</Text>
                                  </TouchableOpacity>
                                </View>
                              ) : (
                                <View style={{ flex: 1, alignItems: 'flex-start', paddingHorizontal: 12 }}>
                                  <Text style={{ fontSize: 13, color: COLORS.textMuted, fontStyle: 'italic' }}>Closed</Text>
                                </View>
                              )}
                  ...
                  {activeTab === 'media' && (
                    <View style={{ gap: 28, paddingBottom: 32, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight }}>
                      <SectionHeading
                        title="Your photos"
                            ...
                            {isSubmittingVerification ? (
                              ...
                              <Text
                                style={{
                                  ...
                                  fontWeight: '600',
                                  color:
                                    verifLicenseNumber.trim() && licenseDocUri ? '#fff' : COLORS.textMuted,
                                }}
                              >
                                Submit for review
                              </Text>

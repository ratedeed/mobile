...
files = [
    ...
    "ContractorEditProfileScreen.tsx", 
    "ContractorOnboardingScreen.tsx",
    "ReviewScreen.tsx",
    "VerifyEmailChangeScreen.tsx",
    "SavedScreen.tsx",
    "BusinessSearchScreen.tsx",
]
...
def process_file(filepath):
    ...
    content = add_dark(content, 'bg-gray-50', 'dark:bg-neutral-900')
    ...
    content = add_dark(content, 'text-gray-900', 'dark:text-white')
    content = add_dark(content, 'text-gray-700', 'dark:text-neutral-300')
    content = add_dark(content, 'text-gray-600', 'dark:text-neutral-300')
    content = add_dark(content, 'text-gray-500', 'dark:text-neutral-400')
    ...
    content = add_dark(content, 'border-gray-200', 'dark:border-neutral-700')
    content = add_dark(content, 'border-gray-300', 'dark:border-neutral-600')
    
    # Fix modal bg-white
    content = content.replace('bg-white dark:bg-neutral-950 rounded-t-2xl', 'bg-white dark:bg-neutral-800 rounded-t-2xl')
    content = content.replace('bg-white dark:bg-neutral-950 rounded-xl', 'bg-white dark:bg-neutral-800 rounded-xl')
    ...
    content = content.replace('border-indigo-100', 'border-indigo-100 dark:border-indigo-800')
...
for f in files:
    process_file(f"{base}/{f}")

# LoadingScreen.js - special handling for StyleSheet
filepath = f"{base}/LoadingScreen.js"
with open(filepath, "r") as f:
    content = f.read()
...
# Replace hardcoded colors in StyleSheet
content = content.replace("const SPLASH_COLOR = '#ffffff';", "const SPLASH_COLOR = isDark ? '#09090B' : '#ffffff';")
content = content.replace("backgroundColor: SPLASH_COLOR", "backgroundColor: isDark ? '#09090B' : '#ffffff'")
content = content.replace("backgroundColor: '#ffffff'", "backgroundColor: isDark ? '#09090B' : '#ffffff'")
content = content.replace("color: '#111827'", "color: isDark ? '#ffffff' : '#111827'")
content = content.replace("color: '#374151'", "color: isDark ? '#a3a3a3' : '#374151'")
content = content.replace("color: '#6B7280'", "color: isDark ? '#6b7280' : '#6B7280'")

with open(filepath, "w") as f:
    f.write(content)

print("Done: LoadingScreen.js")
